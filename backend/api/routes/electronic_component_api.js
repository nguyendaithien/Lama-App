...
electronic_component_api.get('/list', verify_token, check_required_field_in_query(['project_id']), async (req, res, next) => {
    const {project_id} = req.query
    electronic_component_service
        .get_list_component(project_id)
        .then(data => {
            if (data.length === 0) {
                return res.status(500).json({message: 'list electronic components not found'})
            }
...
electronic_component_api.post('/upload',
    verify_token,
    ...
    async (req, res, next) => {
        ...
        try {
            ...
            const electronic_components = await electronic_component_service.read_excel_file(req.file.filename)
            const result = await electronic_component_service.post_list_component(electronic_components, project_id)
